...
public class CmsStaticContentFileManagerImpl implements FilePut, FileGet, FileRemove {
     ...
     * @param inputStaticContentDataList input static content file list which will get stored
     ...
     * in underlying Infinispan cache tree and will be returned on finding an associated
    ...
    public void removeFiles(final String merchantStoreCode)
        ...
        try {
            cacheManager.getTreeCache().getRoot().remove(getRootName() + merchantStoreCode);
        } catch (final Exception e) {
    ...
    private String getNodePath(final String storeCode, final FileContentType contentType) {
        return storeCode + "/" + contentType.name();
    }
