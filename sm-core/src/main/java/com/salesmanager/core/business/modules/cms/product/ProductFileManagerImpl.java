...
/*				//default small
		...
		uploadImage.uploadProductImage(configuration, productImage, smallContentImage);*/
            ...
            if (!StringUtils.isBlank(slargeImageHeight) && !StringUtils.isBlank(slargeImageWidth)) { //&& !StringUtils.isBlank(ssmallImageHeight) && !StringUtils.isBlank(ssmallImageWidth)) {
                ...
                if (!StringUtils.isBlank(configuration.getProperty(CROP_UPLOADED_IMAGES)) && configuration.getProperty(CROP_UPLOADED_IMAGES).equals(Boolean.TRUE.toString())) {
                    //crop image
                ...
                File tempLarge = File.createTempFile(
                        String.valueOf(productImage.getProduct().getId()) + "tmpLarge", "." + extension);
