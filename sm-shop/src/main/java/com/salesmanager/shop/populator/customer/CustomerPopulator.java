...
public class CustomerPopulator extends AbstractDataPopulator<PersistableCustomer, Customer> {
  ...
  protected static final Logger LOGGER = LoggerFactory.getLogger(CustomerPopulator.class);
  ...
  public Customer populate(PersistableCustomer source, Customer target,
    ...
    if (target.getBilling() == null && source.getBilling() != null) {
      LOGGER.info("Setting default values for billing");
      Billing billing = new Billing();
    ...
    if (source.getRating() != null) {
      target.setCustomerReviewAvg(new BigDecimal(source.getRating()));
    }

    target.setCustomerReviewCount(source.getRatingCount());

    if (target.getDelivery() == null && source.getDelivery() != null) {
      LOGGER.info("Setting default value for delivery");
      Delivery delivery = new Delivery();
    ...
    if (source.getAttributes() != null) {
      for (PersistableCustomerAttribute attr : source.getAttributes()) {
        ...
        if (!Objects.equals(customerOption.getMerchantStore().getId(), store.getId())) {
          throw new RuntimeException("Invalid customer option id ");
        }
