...
public class PersistableProductReviewPopulator extends
  ...
  private static final Logger LOGGER = Logger.getLogger(PersistableProductReviewPopulator.class);
  ...
  public ProductReview populate(PersistableProductReview source,
    ...
    if (customer == null || customer.getMerchantStore().getId().intValue() != store.getId()
        .intValue()) {
      throw new RuntimeException("Invalid customer id for the given store");
    }
    ...
    if (product == null || product.getMerchantStore().getId().intValue() != store.getId()
        .intValue()) {
      throw new RuntimeException("Invalid product id for the given store");
    }
    ...
    if (lang == null) {
      throw new RuntimeException("Invalid language code, use iso codes (en, fr ...)");
    }
