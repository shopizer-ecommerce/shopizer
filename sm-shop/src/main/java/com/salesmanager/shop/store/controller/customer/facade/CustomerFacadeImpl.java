...
public class CustomerFacadeImpl implements CustomerFacade {
        ...
        if (sessionShoppingCart != null) {
          if (customerCart == null) {
            ...
            if (sessionShoppingCart.getCustomerId()
              ...
              LOGGER
                  .debug("Customer shopping cart as well session cart is available, merging carts");
  ...
  /**
   * <p> Method to check if given user exists for given username under given store. System treat
   * username as unique for a given store, customer is not allowed to use same username twice for a
   * given store, however it can be used for different stores.</p>
   *
  ...
  public boolean checkIfUserExists(final String userName, final MerchantStore store)
    ...
    if (StringUtils.isNotBlank(userName) && store != null) {
      ...
      LOGGER
          .info("No customer found with userName {} for store {} ", userName, store.getStorename());
      return false;
